...
% Make position index
d.maxPosition = single((p.numLatPix/2 - 1/2)*p.pixSize);
d.minPosition = single(-(p.numLatPix/2 - 1/2)*p.pixSize);
d.posIdx = single(d.minPosition:p.pixSize:d.maxPosition);

% Make z position index
d.maxZPosition = single((p.numZPix/2 - 1/2)*p.pixSize);
d.minZPosition = single(-(p.numZPix/2 - 1/2)*p.pixSize);
