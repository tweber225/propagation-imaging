...
p.numLatPix = 20*p.overSampling;       % number pixels per lateral dimension
p.numZPix = 200;
...
p.pinholeDiam = 2*1.8;  % microns, in sample
p.numPinholes = 4;
...
p.pinholeLocations = p.firstPinholeDepth:p.pinholeSeparation:(p.pinholeSeparation*(p.numPinholes-1) + p.firstPinholeDepth);